...
| repository root | GREEN | `Makefile`, `docker-compose.yml`, `docker-compose.prod.yml`, `Dockerfile`, `Caddyfile`, `README.md`, `.golangci.yml`, `go.mod`. The two production files are named here rather than left to the row's general GREEN because they are read as a pair with `scripts/deploy/` above, which is not GREEN. This file and `CLAUDE.md` files in subdirectories are YELLOW: they are instructions to you, so changing them changes your own brief |
