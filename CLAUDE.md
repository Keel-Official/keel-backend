...
| `docs/api/` | YELLOW | the contract. A change bumps its version and regenerates the mocks; DEC-003's freeze conditions govern when it stops moving. ONE FILE IN IT IS NOT THE CONTRACT AND IS NOT YELLOW: see the row below |
| `docs/api/Keel_PRD.md` | RED in effect since 3 September 2026, AND THE ZONE WORD IS AL'S TO SET | an input from outside, the same class as `docs/context/`: section 9 holds the acceptance criteria the work is scored against, section 4 the requirements, section 12 the scope cutting order. It is the file that says what FINISHED means, and Claude is one of the two things it measures. It was writable until 3 September 2026 for no better reason than that it was filed beside the contract. ENFORCED in both the deny list and the hook, which Claude added, because tightening is Claude's direction. THE ENFORCEMENT IS NOT THE DECISION: moving this file to `docs/context/` where its class belongs, or leaving it here with a zone of its own, is a reclassification and reclassifications are Al's, as DEC-008 established for `compute.go`. Until Al settles it, this row records a lock that runs ahead of a decision, which is the honest state and not a tidy one. Reading stays open, and reporting where the criteria and the repository disagree is the job. P2-27 and P2-28 in `scripts/audit-verification.sh` are the two halves |
