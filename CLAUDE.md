...
| `scripts/vps-deploy/` | GREEN to PREPARE, RED to APPLY | the runbook and the remote script that put the API on a host. Claude writes the runbook, `docker-compose.prod.yml`, the `Caddyfile`, `deploy-remote.sh` and the deploy job; Al owns the box, its DNS, its database password and the SSH key, and Al applies them. THE APPLY HALF IS NOT A COURTESY: Claude must never provision the host, hold a key to it, or run `deploy-remote.sh` against anything, and that is the same reason `scripts/s3-archive/` reads this way. ADDED 11 September 2026 with the files themselves, so the directory has never existed without a row. It would have inherited GREEN from `scripts/` in silence otherwise, which is the cost the 26 August inheritance rule names at the top of this file, and GREEN is the wrong word for a directory whose whole content is instructions for touching production |
