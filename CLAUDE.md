...
| `docs/context/` | RED | inputs from outside, the SoW and the execution plan. Read them, do not write them. ENFORCED since 25 August 2026, same two files. `docs/methodology/` is red and deliberately NOT enforced, because the map gives Claude a job inside it and a lock there would refuse the work the map assigns |
