...
| `internal/domain/compute.go` | YELLOW | moved from RED on 25 August 2026, see DEC-00X. THE ORDERING RULE REPLACES THE LOCK: a function may only be written after its expected values exist in `testdata/fixtures`. Fixtures are never adjusted to match code |
