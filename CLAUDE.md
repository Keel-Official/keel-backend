...
| `scripts/deploy/` | GREEN to PREPARE, RED to APPLY | the runbook and the dump script that put the API on a host and keep it there. Claude writes `RUNBOOK.md`, `dump-database.sh`, `docker-compose.prod.yml`, `nginx-keel.conf` and the deploy job; Al owns the box, its DNS, its database password and the SSH key, and Al applies them. `nginx-keel.conf` REPLACED the root `Caddyfile` on 11 September 2026, when Caddy was removed from the stack because the box already runs nginx, and the new file sits in THIS directory rather than at the root because it is installed into `/etc/nginx` by hand and is therefore prepare-and-apply in a way the compose file is not. THE APPLY HALF IS NOT A COURTESY: Claude must never provision the host, hold a key to it, or run a deploy against anything, and that is the same reason `scripts/s3-archive/` reads this way. It is also why the one-time migration in section 3.5 of that runbook is written as Al's to run rather than automated into the deploy job. ADDED 11 September 2026 with the files themselves, so the directory has never existed without a row; it would have inherited GREEN from `scripts/` in silence otherwise, which is the cost the 26 August inheritance rule names at the top of this file, and GREEN is the wrong word for a directory whose whole content is instructions for touching production. THE PATH WAS `scripts/vps-deploy/` FOR ONE COMMIT, ON THE SAME DAY, and this row moved with the files rather than being left pointing at a directory holding nothing, which is the stale-lock pattern this file has already paid for once |
...
| `docs/api/` | YELLOW | the contract. A change bumps its version and regenerates the mocks; DEC-003's freeze conditions govern when it stops moving. It held one file that was not the contract until 5 September 2026, when `Keel_PRD.md` moved to `docs/context/` where its class belongs. The whole directory is YELLOW again |
...
| `docs/report/` | YELLOW | the client-facing backtest report, and it is the one D2 asks a stranger to read. Claude drafts the structure, the tables and the reproduction steps; every claim about what a number MEANS is Al's. Every number in it cites an evidence file that already exists, and it QUOTES `docs/methodology/` rather than restating a definition in its own words, because a second home for a definition drifts and this one would drift in front of the client. ADDED 5 September 2026 on Al's ratification: the report had no row at all, so the paid write-up of the paid deliverable was a path nobody had agreed could be written, and P2-9 could not see it because the directory did not exist yet |
| `docs/context/` | RED | inputs from outside: the SoW, the execution plan, and since 5 September 2026 the PRD. Read them, do not write them. ENFORCED in all three. `docs/methodology/` is red and deliberately NOT enforced, because the map gives Claude a job inside it and a lock there would refuse the work the map assigns. ONE FILE IN HERE IS TRACKED and the rest are not, which is the row below |
| `docs/context/Keel_PRD.md` | RED, and the zone word was SETTLED 5 September 2026 | it is governed by the row above and needs no zone of its own any more. This row is kept for the history, which is the part worth keeping. Section 9 holds the acceptance criteria the work is scored against, section 4 the requirements, section 12 the scope cutting order. It is the file that says what FINISHED means, and Claude is one of the two things it measures, which is why Claude locked it on 3 September and it then sat for two days with a lock and no decision. THE ENFORCEMENT WAS NEVER THE DECISION: tightening is Claude's direction and Claude took it, but moving the file is a reclassification and reclassifications are Al's, as DEC-008 established for `compute.go`. **THE MOVE ALONE WOULD HAVE DELETED IT.** `.gitignore` excludes this directory wholesale, so the file is kept by one negation line that landed in its own commit BEFORE the move. Without it a clone would have stopped carrying the definition of done, and the 2 September breakdown's false claim that this file is gitignored and unreadable would have become true, which is how the paraphrase that cost three plans got written in the first place. **THREE ANCHORS STILL NAME THE OLD PATH**: two deny list entries and one alternative in the hook's `zone_any`. They now guard a path holding no file. They STAY, because removing a deny rule or narrowing a hook pattern is a LOOSENING and loosenings are Al's, and the new path is already covered by the `docs/context` clause in both, so nothing is unlocked by leaving them. P2-27 and P2-28 in `scripts/audit-verification.sh` are re-anchored to the new path, because a probe that tests a path holding nothing is the stale-lock pattern this file has already paid for once |
...
| repository root | GREEN | `Makefile`, `docker-compose.yml`, `docker-compose.prod.yml`, `Dockerfile`, `README.md`, `env.example`, `.golangci.yml`, `go.mod`. `Caddyfile` WAS NAMED HERE UNTIL 11 SEPTEMBER 2026 and the file is deleted, not moved: its replacement is `scripts/deploy/nginx-keel.conf`, which the row above governs. The name is struck rather than left in place because a row naming a file that no clone contains is the stale-lock pattern at the top of this document, and this list is read as the inventory of what the root holds. The one production file left at the root is named here rather than left to the row's general GREEN because they are read as a pair with `scripts/deploy/` above, which is not GREEN. This file and `CLAUDE.md` files in subdirectories are YELLOW: they are instructions to you, so changing them changes your own brief |
